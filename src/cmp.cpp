...
struct DiffScan : thread<DiffScan>
{
    ...
    void thread(void)
    {
        ...
        while (f_busy)
        {
            ...
            pos_delta += delta;
            if (delta < F[0].textlen)
            {
                break;
            }
            for (i = 0; i < F_num; i++)
            {
                F[i].MoveFilepos(delta);
            }
        }
...
int __stdcall WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
    ...
    for (i = 1; i < Min(DIM(F) + 1, Max(2, argc)); i++)
    {
        ...
        if (F[i - 1].Open(fil1) == 0)
        {
            return 1;
        }
        if (F[i - 1].F1size > 0xFFFFFFFFU)
        {
    ...
    HICON ico = LoadIcon(hInstance, MAKEINTRESOURCE(500));
...
#define GCL_HICON (-14)
    ...
    for (j = 15; j != -1; j--)
    {
        ...
        for (i = 0; i < F_num; i++)
        {
            WX += F[i].Calc_WCX(mBX, lf.f_addr64, (i != F_num - 1)) * ch1.wmax;
        }
    ...
    if (lf.BX * lf.BY > hexfile::datalen - hexfile::datalign)
    {
        lf.BX = (hexfile::datalen - hexfile::datalign) / lf.BY;
    }
            ...
            case WM_SYSKEYDOWN:
                curtim = GetTickCount();
                {
                    ...
                    switch (c)
                    {
                        ...
                        case 0x00BD:
                        case 0x006D: // '-',Gray-
                            if ((ctr) || (!alt)) if (lf.lf.lfHeight < -1)
                                {
                                    lf.lf.lfHeight++;
                                }
                            if ((!ctr) || (alt)) if (lf.lf.lfWidth < -1)
                                {
                                    lf.lf.lfWidth++;
                                }
                            goto Restart;
